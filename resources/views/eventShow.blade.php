<x-main-layout>
    <div class="m-2 p-2 flex justify-between">
        <h3 class="mb-4 text-3xl font-bold text-indigo-700">{{ $event->title }}</h3>
        <div class="flex space-x-2">
            From:
            <span class="mx-2">{{ $event->start_date->format('Y/m/d') }}</span> | <span class="mx-2">{{ $event->end_date->format('Y/m/d') }}</span>
        </div>
    </div>
    <div class="mb-16 flex flex-wrap">
        <div class="mb-6 w-full shrink-0 grow-0 basis-auto lg:mb-0 lg:w-6/12 lg:pr-6">
            <div class="flex flex-col">
                ...
                <div class="flex space-x-2 p-4" x-data="{
                    ...
                    <button type="button" @click="onHandleLike" class="text-white  focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" :class="eventLike ? 'bg-red-500 hover:bg-red-800' : 'bg-slate-500 hover:bg-slate-700'">
                    ...
                    <button type="button" @click="onHandleSavedEvent" class="text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" :class="savedEvent ? 'bg-blue-500 hover:bg-blue-600' : 'bg-slate-500 hover:bg-slate-700'">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h3.5 mr-2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" />
                        </svg>
                        Save
                    </button>
                    <button type="button" @click="onHandleAttending" class="text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" :class="attending ? 'bg-green-500 hover:bg-green-700' : 'bg-slate-500 hover:bg-slate-700'">
                        Attending
                ...
                <div class="flex flex-col p-4">
                    <span class="text-indigo-600 font-semibold">主催者情報</span>
        ...
        <div class="w-full shrink-0 grow-0 lg:w-6/12 lg:pl-6 bg-slate-50 rounded-md p-2">
            <p class="mb-6 text-sm text-yellow-600 dark:text-neutral-400">
                Start: <u>{{$event->toArray()['start_date']}}</u> at <time>{{ $event->start_time }}</time>
            </p>
            <p>
            ...
            <div class="container d-flex justify-content-center align-items-center w-50 mt-6 bg-slate-200 p-4 rounded-md">
                <div>
                    <form action="{{ route('events.comments', $event->id) }}" class="flex justify-between space-x-2" method="POST">
                        ...
                        <button type="submit" class="w-1/3 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5  text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            コメントする
                        </button>
                    </form>
                </div>
