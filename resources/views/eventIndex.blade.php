            ...
            <div class="grid grid-cols-1 gap-8 mt-8 md:mt-16 md:grid-cols-2">
                ...
                <div class="lg:flex bg-slate-100 rounded-md">
                    ...
                    <div class="flex flex-col justify-between py-6 lg:mx-6">
                        ...
                        <span class="text-sm text-white dark:text-gray-300 bg-indigo-400 rounded-md p-2">開催国: {{$event->country->name}}</span>
                        <span class="text-sm text-gray-500 dark:text-gray-300 rounded-md">開催場所: {{$event->address}}</span>
                        <span class="text-sm text-gray-500 dark:text-gray-300">開催日: {{$event->start_date}}</span>
                        <span class="flex flex-wrap space-x-2">
                            @foreach($event->tags as $tag)
                            <p class="text-sm p-1 bg-red-200 rounded-md">{{$tag->name}}</p>
                            @endforeach
                        </span>
                    </div>
                </div>
